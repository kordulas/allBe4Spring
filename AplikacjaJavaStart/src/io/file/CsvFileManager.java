...
import exception.InvalidDataExpection;
import libary.model.*;
import java.io.*;
...
public class CsvFileManager implements FileManager {
    ...
    private <T extends CsvConvertible> void exportToCsv(Collection<T> collection, String fileName) {
        try (FileWriter fileWriter = new FileWriter(fileName);
            ...
            for (T kolekcja : collection) {
                bufferedWriter.write(kolekcja.toCsv());
    ...
    private Publication createObjectFromString(String csvText) {
        ...
        throw new InvalidDataExpection("Nieznany typ publikacji: " + type);
    }
    ...
    private void importPublications(Library library) {
        try (BufferedReader br = new BufferedReader(new FileReader(PUBLICATIONS_FILE_NAME))) {
            br.lines()
